...
- 访达相关/finder相关
  ...
  - 处理特别文件夹(mac)
	...
	- chflags uchg tmp_dir  等效于「显示简介」-「已锁定」
	
	  - **含义**：`uchg` 代表 **user change-immutable flag**（用户层级不可变标志）。
	
	    **具体表现**：
	
	    - 一旦设置，即使你是该目录的所有者（Owner），也**无法删除、重命名、移动**该目录，也**无法在里面创建或删除文件**。
	
	    - 在 macOS 访达（Finder）中，你会看到该文件夹图标上出现了一个**小锁头**。
	
	  **解锁命令**：`chflags nouchg tmp_dir`（在前面加个 `no`）。
	
	- chflags hidden tmp_dir
	
	  - **含义**：`hidden` 标志会将该目录在图形界面中隐藏。
	
	    **具体表现**：
	
	    - **访达（Finder）**：该文件夹将不再显示在窗口中，普通用户通过 UI 找不到它。
	    - **终端（Terminal）**：使用普通的 `ls` 看不到它，但使用 `ls -l` 或 `ls -LO` 仍然可以查看到它的标志位。
	    - 这与 Linux 风格的“点开头隐藏”（如 `.tmp_dir`）不同，它不需要修改文件夹的名字就能实现隐藏。
	
	  **取消隐藏**：`chflags nohidden tmp_dir`。
...
1. $0 是shell脚本本身名字，$1是shell脚本/函数第一个参数，以此类推。注意c语言的int main(int argc, char *argv[])与此类似，argv[0]是程序本身名字，然后就是参数，argc是包含程序本身名的参数数量(>=1)，但是$#不包含程序本身名的参数个数
   ...
   - 有一个特殊用法 "${n:-}", 这是 Shell 的变量扩展语法。n 表示脚本接收到的第n个参数。:- 的作用是：如果 $n 未设置或为空，则取其后的默认值（这里是空字符串）。示例: "${1:-abc}", 即第1个参数默认参数为abc
   ```
...
- 准备
  - 排骨(纯排25一斤)
  - 大蒜 姜 生抽 老抽 冰糖 鸡精 辣椒 花椒 藤椒
  - 八角 香叶 桂皮 小茴香
- 实操
  - 排骨焯水
  - 锅中放油，放入所有佐料(冰糖除外)， 放入排骨炒
  - 炒了放入开水， 移至高压锅
  - 高压锅上汽煮13分钟，注意一定不能太久，不然压废
  - 放入炒锅，大火收汁，放入鸡精，起锅
